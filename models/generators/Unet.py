import torch
import torch.nn as nn
import torch.nn.functional as F

import Utils

class Unet(nn.Module):
    def __init__(self, opt, ngf, nc_in=3, nc_out=3):

        super(Unet, self).__init__()
        self.inc = inconv(nc_in, ngf)
        self.down1 = down(ngf, ngf*2)
        self.down2 = down(ngf*2, ngf*4)
        self.down3 = down(ngf*4, ngf*8)
        self.down4 = down(ngf*8, ngf*8)

        fmap_shape = (opt.input_width//16)*(opt.input_height//16)
        noise_channels = 2
        self.fc_noise = nn.Linear(opt.nz, noise_channels*fmap_shape)

        self.up1 = up(ngf*16 + noise_channels, ngf*4)
        self.up2 = up(ngf*8, ngf*2)
        self.up3 = up(ngf*4, ngf)
        self.up4 = up(ngf*2, ngf)
        self.outc = outconv(ngf, nc_out)

        if opt.generator_activation == "sigmoid":
            self.output_activation = nn.Sigmoid()
        elif opt.generator_activation == "relu":
            self.output_activation = nn.ReLU()
        else:
            print("WARNING: Using Unet without output activation")
            self.output_activation = None

        if hasattr(opt, "generator_mask"):
            assert(opt.generator_mask == 0 or nc_out == 1)
            assert(opt.generator_mask == 0 or opt.generator_activation == "sigmoid")
            self.mask = opt.generator_mask
        else:
            self.mask = False

    def forward(self, input):
        cond = input[0]
        noise = input[1]

        # Downward
        x1 = self.inc(cond)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        x5 = self.down4(x4)

        # FC to append noise channels to feature map
        noise_fmap_shape = [x5.shape[0], -1, x5.shape[2], x5.shape[3]]
        noise_fmap = self.fc_noise(noise).view(noise_fmap_shape)
        x5 = torch.cat([x5, noise_fmap], dim=1)

        # Upward
        x = self.up1(x5, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        x = self.outc(x)

        if self.output_activation != None:
            x = self.output_activation(x)

        if self.mask:
            # Denormalise audio mixture input
            original_cond = Utils.denormalise_spectrogram_torch(cond)

            # Compute acc and voice based on mask and unnormalised mixture spectrogram, then renormalise
            acc = Utils.normalise_spectrogram_torch(original_cond * x)
            voice = Utils.normalise_spectrogram_torch(original_cond * (1.0 - x))

            x = torch.cat([acc, voice], dim=1)

        return torch.cat([cond, x], dim=1)

class double_conv(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch):
        super(double_conv, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )

    def forward(self, x):
        x = self.conv(x)
        return x

class inconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(inconv, self).__init__()
        self.conv = double_conv(in_ch, out_ch)

    def forward(self, x):
        x = self.conv(x)
        return x


class down(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(down, self).__init__()
        self.mpconv = nn.Sequential(
            nn.MaxPool2d(2),
            double_conv(in_ch, out_ch)
        )

    def forward(self, x):
        x = self.mpconv(x)
        return x


class up(nn.Module):
    def __init__(self, in_ch, out_ch, bilinear=True):
        super(up, self).__init__()

        #  would be a nice idea if the upsampling could be learned too,
        #  but my machine do not have enough memory to handle all those weights
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        else:
            self.up = nn.ConvTranspose2d(in_ch // 2, in_ch // 2, 2, stride=2)

        self.conv = double_conv(in_ch, out_ch)

    def forward(self, x1, x2):
        x1 = self.up(x1)

        # input is CHW
        diffY = x2.size()[2] - x1.size()[2]
        diffX = x2.size()[3] - x1.size()[3]

        x1 = F.pad(x1, (diffX // 2, diffX - diffX // 2,
                        diffY // 2, diffY - diffY // 2))

        # for padding issues, see
        # https://github.com/HaiyongJiang/U-Net-Pytorch-Unstructured-Buggy/commit/0e854509c2cea854e247a9c615f175f76fbb2e3a
        # https://github.com/xiaopeng-liao/Pytorch-UNet/commit/8ebac70e633bac59fc22bb5195e513d5832fb3bd

        x = torch.cat([x2, x1], dim=1)
        x = self.conv(x)
        return x


class outconv(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(outconv, self).__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, 1)

    def forward(self, x):
        x = self.conv(x)
        return x